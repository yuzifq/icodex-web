...
import {
  ...
  getModelReasoningEffortSettings,
...
const RUNTIME_CONFIG_BY_CONTEXT_STORAGE_KEY = 'codex-web-local.runtime-config-by-context.v1'
const LEGACY_SELECTED_MODEL_STORAGE_KEY = 'codex-web-local.selected-model-id.v1'
const REMEMBER_LAST_SELECTED_MODEL_STORAGE_KEY = 'codex-web-local.remember-last-selected-model.v1'
...
function loadRuntimeConfigMap(): Record<string, CodexRuntimeConfig> {
  if (typeof window === 'undefined') return createStringKeyedRecord<CodexRuntimeConfig>()
  try {
    const raw = window.localStorage.getItem(RUNTIME_CONFIG_BY_CONTEXT_STORAGE_KEY)
    if (!raw) return createStringKeyedRecord<CodexRuntimeConfig>()
    const parsed = JSON.parse(raw) as unknown
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return createStringKeyedRecord<CodexRuntimeConfig>()

    const next = createStringKeyedRecord<CodexRuntimeConfig>()
    for (const [contextId, value] of Object.entries(parsed as Record<string, unknown>)) {
      if (!contextId) continue
      const config = normalizeCodexRuntimeConfig(value)
      if (config) next[contextId] = config
    }
    return next
  } catch {
    return createStringKeyedRecord<CodexRuntimeConfig>()
  }
}

function readSelectedRuntimeConfig(state: Record<string, CodexRuntimeConfig>, threadId: string): CodexRuntimeConfig {
  const contextId = toThreadContextId(threadId)
  return state[contextId] ?? state[NEW_THREAD_COLLABORATION_MODE_CONTEXT] ?? runtimeConfigForAccessMode('request')
}

function writeSelectedRuntimeConfigForContext(
  state: Record<string, CodexRuntimeConfig>,
  threadId: string,
  config: CodexRuntimeConfig,
): Record<string, CodexRuntimeConfig> {
  const next = cloneStringKeyedRecord(state)
  next[toThreadContextId(threadId)] = config
  return next
}

function saveRuntimeConfigMap(state: Record<string, CodexRuntimeConfig>): void {
  if (typeof window === 'undefined') return
  try {
    if (Object.keys(state).length === 0) {
      window.localStorage.removeItem(RUNTIME_CONFIG_BY_CONTEXT_STORAGE_KEY)
    } else {
      window.localStorage.setItem(RUNTIME_CONFIG_BY_CONTEXT_STORAGE_KEY, JSON.stringify(state))
    }
  } catch {
    // Keep the in-memory selection working if localStorage writes fail.
  }
}

function loadRememberLastSelectedModel(): boolean {
  if (typeof window === 'undefined') return true
  return window.localStorage.getItem(REMEMBER_LAST_SELECTED_MODEL_STORAGE_KEY) !== 'false'
}

function saveRememberLastSelectedModel(enabled: boolean): void {
  if (typeof window === 'undefined') return
  window.localStorage.setItem(REMEMBER_LAST_SELECTED_MODEL_STORAGE_KEY, String(enabled))
}
...
export function useDesktopState() {
  ...
  const selectedRuntimeConfigByContext = ref<Record<string, CodexRuntimeConfig>>(loadRuntimeConfigMap())
  const rememberLastSelectedModel = ref(loadRememberLastSelectedModel())
  ...
  const selectedRuntimeConfig = ref(readSelectedRuntimeConfig(selectedRuntimeConfigByContext.value, selectedThreadId.value))
  const selectedReasoningEffort = ref<ReasoningEffort | ''>('medium')
  const availableReasoningEfforts = ref<ReasoningEffort[]>([...REASONING_EFFORT_OPTIONS])
  const selectedModelSupportsFast = ref(false)
  ...
  function ensureAvailableModelIds(...modelIds: string[]): void {
    ...
    for (const modelId of modelIds) {
      ...
      if (normalizedModelId && normalizedModelId !== 'codex-auto-review' && !nextModelIds.includes(normalizedModelId)) {
  ...
  async function refreshReasoningEffortsForModel(modelId: string): Promise<void> {
    const normalizedModelId = modelId.trim()
    if (!normalizedModelId) return
    try {
      const settings = await getModelReasoningEffortSettings(normalizedModelId)
      if (selectedModelId.value.trim() !== normalizedModelId || settings.efforts.length === 0) return
      availableReasoningEfforts.value = settings.efforts
      selectedModelSupportsFast.value = settings.supportsFast && settings.efforts.includes('xhigh')
      if (!settings.efforts.includes(selectedReasoningEffort.value as ReasoningEffort)) {
        selectedReasoningEffort.value = settings.defaultEffort || settings.efforts[0] || ''
      }
    } catch {}
  }
  ...
  function setSelectedThreadId(nextThreadId: string, options: { persist?: boolean } = {}): void {
    ...
    const threadReasoningEffort = threadReasoningEffortByThreadId.value[nextThreadId]
    if (threadReasoningEffort) selectedReasoningEffort.value = threadReasoningEffort
    selectedRuntimeConfig.value = readSelectedRuntimeConfig(selectedRuntimeConfigByContext.value, nextThreadId)
  ...
  function setSelectedModelIdForThread(threadId: string, modelId: string): void {
    ...
    if (threadId.trim() === selectedThreadId.value) {
      ...
      void refreshReasoningEffortsForModel(selectedModelId.value)
    ...
    if (rememberLastSelectedModel.value && normalizedModelId) {
      const providerContextId = toProviderModelContextId(readProviderIdForThread(threadId))
      const nextModelMap = cloneStringKeyedRecord(selectedModelIdByContext.value)
      if (providerContextId) {
    ...
    if (threadId.trim() === selectedThreadId.value) {
      void refreshReasoningEffortsForModel(normalizedModelId)
    }
  }

  function setRememberLastSelectedModel(enabled: boolean): void {
    rememberLastSelectedModel.value = enabled
    saveRememberLastSelectedModel(enabled)
    if (enabled && selectedModelId.value.trim()) {
      setSelectedModelIdForThread(selectedThreadId.value, selectedModelId.value)
    }
  }
  ...
  function setThreadReasoningEffort(threadId: string, effort: ReasoningEffort | null): void {
    const normalizedThreadId = threadId.trim()
    if (!normalizedThreadId || !effort || !REASONING_EFFORT_OPTIONS.includes(effort)) return
    threadReasoningEffortByThreadId.value = {
      ...threadReasoningEffortByThreadId.value,
      [normalizedThreadId]: effort,
    }
    if (selectedThreadId.value === normalizedThreadId) selectedReasoningEffort.value = effort
  }

  function setSelectedRuntimeConfigForThread(threadId: string, config: CodexRuntimeConfig): void {
    const normalizedConfig = normalizeCodexRuntimeConfig(config)
    if (!normalizedConfig) return
    const nextConfigMap = writeSelectedRuntimeConfigForContext(
      selectedRuntimeConfigByContext.value,
      threadId,
      normalizedConfig,
    )
    selectedRuntimeConfigByContext.value = nextConfigMap
    if (threadId.trim() === selectedThreadId.value) {
      selectedRuntimeConfig.value = normalizedConfig
    }
    saveRuntimeConfigMap(nextConfigMap)
  }

  function setThreadRuntimeConfig(threadId: string, config: CodexRuntimeConfig | null): void {
    if (!threadId.trim() || !config) return
    setSelectedRuntimeConfigForThread(threadId, config)
  }

  function setSelectedRuntimeConfig(config: CodexRuntimeConfig): void {
    setSelectedRuntimeConfigForThread(selectedThreadId.value, config)
  }
  ...
  async function retryPendingTurnWithFallback(threadId: string): Promise<void> {
    ...
    try {
      ...
      if (resumedThreadById.value[threadId] !== true) {
        ...
        setThreadRuntimeConfig(threadId, resumedThread.runtimeConfig)
  ...
  async function refreshModelPreferences(options?: { providerChanged?: boolean; includeProviderModels?: boolean }): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        !threadReasoningEffortByThreadId.value[selectedThreadId.value]
      ) {
        selectedReasoningEffort.value = currentConfig.reasoningEffort
      }
      const currentRuntimeContextId = toThreadContextId(selectedThreadId.value)
      if (currentConfig.runtimeConfig && !selectedRuntimeConfigByContext.value[currentRuntimeContextId]) {
        setSelectedRuntimeConfigForThread(selectedThreadId.value, currentConfig.runtimeConfig)
      }
  ...
  async function loadMessages(threadId: string, options: { silent?: boolean } = {}) {
    ...
    const loadPromise = (async () => {
      ...
      setThreadReasoningEffort(threadId, resumedThread?.reasoningEffort ?? null)
      setThreadRuntimeConfig(threadId, resumedThread?.runtimeConfig ?? null)
      if (resumedThread) {
        resumedThreadById.value = {
          ...resumedThreadById.value,
          [threadId]: true,
  ...
  async function forkThreadById(threadId: string): Promise<string> {
    ...
    try {
      ...
      setThreadRuntimeConfig(nextThreadId, forkedThread.runtimeConfig)
      resumedThreadById.value = {
        ...resumedThreadById.value,
        [nextThreadId]: true,
      }
  ...
  async function forkThreadFromTurn(threadId: string, turnIndex: number): Promise<string> {
    ...
    try {
      ...
      setThreadRuntimeConfig(forkedThreadId, forked.runtimeConfig)
  ...
  async function sendMessageToNewThread(
    ...
    try {
      try {
        ...
        setThreadRuntimeConfig(threadId, startedThread.runtimeConfig ?? runtimeConfig ?? selectedRuntimeConfig.value)
        ...
        if (selectedModel && selectedModel !== MODEL_FALLBACK_ID && isUnsupportedChatGptModelError(unknownError)) {
          ...
          setThreadRuntimeConfig(threadId, fallbackThread.runtimeConfig ?? runtimeConfig ?? selectedRuntimeConfig.value)
          setSelectedCollaborationModeForThread(threadId, selectedMode)
        } else {
          throw unknownError
        }
  ...
  async function startTurnForThread(
    ...
    try {
      if (resumedThreadById.value[threadId] !== true) {
        ...
        setThreadReasoningEffort(threadId, resumedThread.reasoningEffort)
        setThreadRuntimeConfig(threadId, resumedThread.runtimeConfig)
        resumedThreadById.value = {
          ...resumedThreadById.value,
          [threadId]: true,
  ...
  return {
    ...
    setRememberLastSelectedModel,
    setSelectedModelId,
    setSelectedRuntimeConfig,

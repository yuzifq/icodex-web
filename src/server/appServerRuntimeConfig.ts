...
export function buildAppServerArgs(): string[] {
  ...
  const approvalPolicy = parseCodexApprovalPolicy(normalizeRuntimeValue(process.env.CODEXUI_APPROVAL_POLICY))
  const sandboxMode = parseCodexSandboxMode(normalizeRuntimeValue(process.env.CODEXUI_SANDBOX_MODE))

  // Do not replace a resumed conversation's CLI-saved access settings with UI defaults.
  if (approvalPolicy) {
  ...
  args.push('-c', `features.memories=${readMemoriesFromEnv() ? 'true' : 'false'}`)
  return args
}

...
import { readFile, readdir, rm, mkdir, stat, lstat } from 'node:fs/promises'
...
type GithubRepositoryApiRecord = {
  name?: string
  full_name?: string
  private?: boolean
  size?: number
...
type StartupSyncStatus = {
  inProgress: boolean
  mode: 'repository-selection-required' | 'authenticated-repository-sync' | 'idle'
  branch: string
  lastAction: string
  lastRunAtIso: string
...
async function resolveGithubDeviceClientId(state?: SkillsSyncState): Promise<string> {
  const environmentClientId = process.env.CODEXUI_GITHUB_CLIENT_ID?.trim()
  if (environmentClientId) return environmentClientId
  const current = state ?? await readSkillsSyncState()
  return current.githubClientId?.trim() ?? ''
}

function normalizeGithubClientId(value: unknown): string {
  if (typeof value !== 'string') return ''
  const clientId = value.trim()
  if (clientId.length > 200 || /\s/u.test(clientId)) return ''
  return clientId
}
...
export function classifyGithubDeviceTokenResponse(payload: GithubTokenResponse): GithubDeviceLoginResult {
  if (payload.access_token) {
    return { token: payload.access_token, error: null, pending: false, retryAfterSeconds: 0 }
  }
  const error = payload.error || 'unknown_error'
  return {
    token: null,
    error: payload.error_description || error,
    pending: error === 'authorization_pending' || error === 'slow_down',
    retryAfterSeconds: error === 'slow_down' ? 5 : 0,
  }
}

async function completeGithubDeviceLogin(clientId: string, deviceCode: string): Promise<GithubDeviceLoginResult> {
  ...
  return classifyGithubDeviceTokenResponse(await resp.json() as GithubTokenResponse)
}
...
export async function listOwnedGithubRepositories(token: string, username: string): Promise<GithubRepositorySummary[]> {
  const records = await getGithubJson<GithubRepositoryApiRecord[]>(
    'https://api.github.com/user/repos?affiliation=owner&sort=updated&direction=desc&per_page=100',
    token,
  )
  return records
    .filter((record) => record.owner?.login?.toLowerCase() === username.toLowerCase())
    .filter((record) => record.permissions?.push !== false && record.archived !== true && record.disabled !== true)
    .map((record) => ({
      owner: record.owner?.login ?? username,
      name: record.name ?? '',
      fullName: record.full_name ?? `${username}/${record.name ?? ''}`,
      private: record.private === true,
      empty: (record.size ?? 0) === 0,
      defaultBranch: record.default_branch || DEFAULT_SYNC_BRANCH,
...
export async function validateSkillsRepository(
  token: string,
  username: string,
  owner: string,
  name: string,
): Promise<{ empty: boolean; branch: string }> {
  const details = await getGithubRepositoryDetails(token, owner, name)
  if (details.owner?.login?.toLowerCase() !== username.toLowerCase()) {
    throw new Error('Only repositories owned by the logged-in GitHub account can be selected')
  }
  if (details.permissions?.push === false) {
    throw new Error('The selected repository does not allow write access')
  }
  if (details.archived === true || details.disabled === true) {
    throw new Error('Archived or disabled repositories cannot be used for Skills sync')
  }

  const branch = details.default_branch || DEFAULT_SYNC_BRANCH
  const rootUrl = `https://api.github.com/repos/${owner}/${name}/contents?ref=${encodeURIComponent(branch)}`
  ...
  if (rootResponse.status === 404 && (details.size ?? 0) === 0) return { empty: true, branch }
  if (!rootResponse.ok) {
    throw new Error(`Failed to inspect the selected repository (${rootResponse.status})`)
  }
  const rootEntries = await rootResponse.json() as Array<{ name?: string; type?: string }>
  const hasManifest = Array.isArray(rootEntries)
    && rootEntries.some((entry) => entry.name === SKILLS_SYNC_MANIFEST_PATH && entry.type === 'file')
  if (!hasManifest) {
    throw new Error('The selected repository is not empty and is not a Skills repository')
  }

  const manifestUrl = `https://api.github.com/repos/${owner}/${name}/contents/${SKILLS_SYNC_MANIFEST_PATH}?ref=${encodeURIComponent(branch)}`
  ...
  if (!manifestResponse.ok) {
    throw new Error(`Failed to validate the selected repository (${manifestResponse.status})`)
  }
  const manifestPayload = await manifestResponse.json() as { type?: string; content?: string }
  if (manifestPayload.type !== 'file' || !manifestPayload.content) {
    throw new Error('The selected repository has an invalid Skills manifest')
  }
  try {
    const manifestText = Buffer.from(manifestPayload.content.replace(/\n/g, ''), 'base64').toString('utf8')
    if (!Array.isArray(JSON.parse(manifestText))) {
...
async function readRemoteSkillsManifest(token: string, repoOwner: string, repoName: string, branch: string): Promise<SyncedSkill[]> {
  const url = `https://api.github.com/repos/${repoOwner}/${repoName}/contents/${SKILLS_SYNC_MANIFEST_PATH}?ref=${encodeURIComponent(branch)}`
  const resp = await fetch(url, {
    headers: {
      Accept: 'application/vnd.github+json',
      Authorization: `Bearer ${token}`,
...
async function writeRemoteSkillsManifest(
  ...
  const baseUrl = `https://api.github.com/repos/${repoOwner}/${repoName}/contents/${SKILLS_SYNC_MANIFEST_PATH}`
  ...
  await getGithubJson(baseUrl, token, 'PUT', {
...
async function pullInstalledSkillsFolderFromRepo(token: string, repoOwner: string, repoName: string, branch: string): Promise<string> {
  const remoteUrl = toGitHubTokenRemote(repoOwner, repoName, token)
...
async function autoPushSyncedSkills(appServer: AppServerLike): Promise<void> {
  const state = await readSkillsSyncState()
  if (!state.githubToken || !state.repoOwner || !state.repoName || !state.repoBranch) return
  const repoDir = getSkillsInstallDir()
  await runCommand('git', ['fetch', 'origin', state.repoBranch], { cwd: repoDir })
  const head = (await runCommandWithOutput('git', ['rev-parse', 'HEAD'], { cwd: repoDir })).trim()
  const originHead = (await runCommandWithOutput('git', ['rev-parse', `origin/${state.repoBranch}`], { cwd: repoDir })).trim()
  ...
  const installedMap = await scanInstalledSkillsFromDisk()
  await writeRemoteSkillsManifest(state.githubToken, state.repoOwner, state.repoName, state.repoBranch, local)
  await syncInstalledSkillsFolderToRepo(state.githubToken, state.repoOwner, state.repoName, state.repoBranch, installedMap)
}
...
async function runSkillsSyncStartup(appServer: AppServerLike): Promise<void> {
  ...
  startupSyncStatus.branch = DEFAULT_SYNC_BRANCH
  try {
    const state = await readSkillsSyncState()
    if (!state.githubToken) {
      startupSyncStatus.mode = 'idle'
      startupSyncStatus.lastAction = 'github-login-required'
      startupSyncStatus.lastSuccessAtIso = new Date().toISOString()
      return
    }
    if (!state.githubUsername || !state.repoOwner || !state.repoName || !state.repoBranch) {
      startupSyncStatus.mode = 'repository-selection-required'
      startupSyncStatus.lastAction = 'select-repository-required'
      startupSyncStatus.lastSuccessAtIso = new Date().toISOString()
      return
    }
    startupSyncStatus.mode = 'authenticated-repository-sync'
    startupSyncStatus.branch = state.repoBranch
    startupSyncStatus.lastAction = 'validate-repository'
    await validateSkillsRepository(state.githubToken, state.githubUsername, state.repoOwner, state.repoName)
    startupSyncStatus.lastAction = 'pull-selected-repository'
    await pullInstalledSkillsFolderFromRepo(state.githubToken, state.repoOwner, state.repoName, state.repoBranch)
    try { await appServer.rpc('skills/list', { forceReload: true }) } catch {}
    startupSyncStatus.lastAction = 'push-selected-repository'
...
async function finalizeGithubLogin(token: string, username: string): Promise<GithubRepositorySummary[]> {
  ...
  startupSyncStatus.lastError = ''
  startupSyncStatus.lastSuccessAtIso = new Date().toISOString()
  return await listOwnedGithubRepositories(token, username)
}

async function pullSelectedSkillsRepository(state: SkillsSyncState, appServer: AppServerLike): Promise<number> {
  if (!state.githubToken || !state.githubUsername) throw new Error('GitHub login required')
  if (!state.repoOwner || !state.repoName || !state.repoBranch) throw new Error('Select a Skills repository first')
  await validateSkillsRepository(state.githubToken, state.githubUsername, state.repoOwner, state.repoName)
  const remote = await readRemoteSkillsManifest(state.githubToken, state.repoOwner, state.repoName, state.repoBranch)
  ...
  for (const [name, localInfo] of localSkills.entries()) {
    if (!remoteNames.has(name)) {
      await rm(localInfo.path.replace(/[\\/]SKILL\.md$/u, ''), { recursive: true, force: true })
    }
  }
...
async function initializeEmptySkillsRepository(
  token: string,
  owner: string,
  name: string,
  branch: string,
  ...
  try {
    ...
    if (!gitInfo.isDirectory() && !gitInfo.isFile()) throw new Error('invalid git directory')
  } catch {
    await runCommand('git', ['init'], { cwd: repoDir })
  }
  await runCommand('git', ['config', 'user.email', 'skills-sync@local'], { cwd: repoDir })
  await runCommand('git', ['config', 'user.name', 'Skills Sync'], { cwd: repoDir })
  await writeFile(join(repoDir, SKILLS_SYNC_MANIFEST_PATH), JSON.stringify(skills, null, 2), 'utf8')
  await runCommand('git', ['add', '-A'], { cwd: repoDir })
  try { await runCommand('git', ['commit', '-m', 'Initialize Skills sync repository'], { cwd: repoDir }) } catch {}
  await runCommand('git', ['branch', '-M', branch], { cwd: repoDir })
  const remoteUrl = toGitHubTokenRemote(owner, name, token)
  try {
    await runCommand('git', ['remote', 'add', 'origin', remoteUrl], { cwd: repoDir })
  } catch {
    await runCommand('git', ['remote', 'set-url', 'origin', remoteUrl], { cwd: repoDir })
  }
  await runCommand('git', ['push', '-u', 'origin', `HEAD:${branch}`], { cwd: repoDir })
}

async function selectSkillsRepository(
  ...
  if (!state.githubToken || !state.githubUsername) throw new Error('GitHub login required')
  const validation = await validateSkillsRepository(state.githubToken, state.githubUsername, owner, name)
  let branch = validation.branch
  let nextState: SkillsSyncState = { ...state, repoOwner: owner, repoName: name, repoBranch: branch }

  if (validation.empty) {
    const local = await collectLocalSyncedSkills(appServer)
    await initializeEmptySkillsRepository(state.githubToken, owner, name, branch, local)
    const initialized = await getGithubRepositoryDetails(state.githubToken, owner, name)
    branch = initialized.default_branch || branch
    nextState = { ...nextState, repoBranch: branch }
    await writeSkillsSyncState(nextState)
    return nextState
  }

  await writeSkillsSyncState(nextState)
  await pullSelectedSkillsRepository(nextState, appServer)
  return nextState
}
...
export async function handleSkillsRoutes(
  ...
  if (req.method === 'GET' && url.pathname === '/codex-api/skills-sync/status') {
    ...
    setJson(res, 200, {
      data: {
        ...
        configured: Boolean(state.githubToken && state.repoOwner && state.repoName && state.repoBranch),
        telemetry: {
  ...
  if (req.method === 'GET' && url.pathname === '/codex-api/skills-sync/github/repositories') {
    try {
      const state = await readSkillsSyncState()
      ...
      const repositories = await listOwnedGithubRepositories(state.githubToken, state.githubUsername)
      setJson(res, 200, { data: { repositories } })
    } catch (error) {
      setJson(res, 502, { error: getErrorMessage(error, 'Failed to list GitHub repositories') })
    }
    return true
  }

  if (req.method === 'POST' && url.pathname === '/codex-api/skills-sync/github/select-repository') {
    try {
      const state = await readSkillsSyncState()
      if (!state.githubToken || !state.githubUsername) {
        setJson(res, 401, { error: 'GitHub login required' })
      ...
      if (!owner || !name) {
        setJson(res, 400, { error: 'Select a repository' })
        return true
      }
      const selected = await selectSkillsRepository(state, owner, name, appServer)
      setJson(res, 200, {
        ok: true,
  ...
  if (req.method === 'POST' && url.pathname === '/codex-api/skills-sync/github/start-login') {
    try {
      const state = await readSkillsSyncState()
      const payload = asRecord(await readJsonBody(req))
      const requestedClientId = normalizeGithubClientId(payload?.clientId)
      const clientId = requestedClientId || await resolveGithubDeviceClientId(state)
      if (!clientId) {
        setJson(res, 400, {
          code: 'github_oauth_client_id_required',
          error: 'GitHub OAuth Client ID is required',
        })
        return true
      }
      const started = await startGithubDeviceLogin(clientId)
      if (requestedClientId && requestedClientId !== state.githubClientId) {
        await writeSkillsSyncState({ ...state, githubClientId: requestedClientId })
      }
  ...
  if (req.method === 'POST' && url.pathname === '/codex-api/skills-sync/github/complete-login') {
    try {
      ...
      const clientId = await resolveGithubDeviceClientId()
      if (!clientId) {
        setJson(res, 400, {
          code: 'github_oauth_client_id_required',
          error: 'GitHub OAuth Client ID is required',
      ...
      const result = await completeGithubDeviceLogin(clientId, deviceCode)
      if (!result.token) {
        setJson(res, 200, {
          ok: false,
      ...
      const repositories = await finalizeGithubLogin(token, username)
      setJson(res, 200, { ok: true, data: { githubUsername: username, repositories } })
    } catch (error) {
      if (getErrorMessage(error, '').toLowerCase().includes('fetch failed')) {
        setJson(res, 200, {
          ok: false,
          pending: true,
          retryAfterSeconds: 5,
          error: 'Temporary GitHub network error; retrying',
        })
        return true
      }
  ...
  if (req.method === 'POST' && url.pathname === '/codex-api/skills-sync/push') {
    try {
      ...
      if (!state.repoOwner || !state.repoName || !state.repoBranch) {
        setJson(res, 400, { error: 'Select a Skills repository first' })
        return true
      }
      await validateSkillsRepository(state.githubToken, state.githubUsername, state.repoOwner, state.repoName)
      const local = await collectLocalSyncedSkills(appServer)
  ...
  if (req.method === 'POST' && url.pathname === '/codex-api/skills-sync/startup-sync') {
    try {
      ...
      await runSkillsSyncStartup(appServer)
      if (startupSyncStatus.lastError) throw new Error(startupSyncStatus.lastError)
      setJson(res, 200, { ok: true })
    } catch (error) {
  ...
  if (req.method === 'POST' && url.pathname === '/codex-api/skills-sync/pull') {
    try {
      ...
      const synced = await pullSelectedSkillsRepository(state, appServer)
      setJson(res, 200, { ok: true, data: { synced } })
    } catch (error) {

      ...
      <div v-else class="directory-section composio-section">
          ...
          <p class="directory-card-description">
            ...
            <div class="directory-chip-row">
              ...
              <span v-if="composioConnectors.length" class="directory-chip">
                {{ composioTotal ? t('Showing {loaded} / {total} connectors', { loaded: composioConnectors.length, total: composioTotal }) : t('Showing {loaded} connectors', { loaded: composioConnectors.length }) }}
              </span>
            </div>
          <div class="directory-card-actions">
        ...
        <div v-else class="directory-grid">
          <article v-for="connector in visibleComposioConnectors" :key="connector.slug" class="directory-card">
            ...
            <div class="directory-chip-row">
              <span class="directory-chip">{{ t('{count} tools', { count: connector.toolsCount }) }}</span>
              <span v-if="connector.triggersCount > 0" class="directory-chip">{{ t('{count} triggers', { count: connector.triggersCount }) }}</span>
        ...
        <template #before-installed>
          <div class="skills-embedded-section">
            <button class="skills-embedded-toggle" type="button" @click="isMcpSectionOpen = !isMcpSectionOpen">
              <span class="skills-embedded-title">{{ t('MCPs') }}({{ visibleMcpServers.length }})</span>
              ...
              <div v-else class="mcp-skill-grid">
                <article v-for="server in visibleMcpServers" :key="server.name">
                  <button class="mcp-skill-card skill-card" type="button" @click="toggleMcpExpanded(server.name)">
                    ...
                    <p class="mcp-skill-meta">{{ t('{count} tools', { count: server.tools.length }) }} · {{ t('{count} resources', { count: server.resources.length + server.resourceTemplates.length }) }}</p>
            ...
            <template v-else-if="selectedComposioDetail">
              ...
              <div class="directory-detail-grid">
                <div class="directory-detail-block">
                  ...
                  <div class="directory-chip-row">
                    <span class="directory-chip">{{ t('{count} tools', { count: selectedComposioDetail.connector.toolsCount }) }}</span>
                    <span v-if="selectedComposioDetail.connector.triggersCount > 0" class="directory-chip">{{ t('{count} triggers', { count: selectedComposioDetail.connector.triggersCount }) }}</span>
...
const tabs: Array<{ id: DirectoryTab; label: string; subtitle: string }> = [
  ...
  { id: 'skills', label: 'Skills', subtitle: 'MCPs first, then installed skills on this machine.' },
]
...
const composioWorkspaceSummary = computed(() => {
  ...
  const parts = [
    ...
    status.defaultOrgId ? t('org {id}', { id: status.defaultOrgId }) : '',
    status.baseUrl || '',
  ].filter(Boolean)
...
function buildComposioTryPrompt(connector: DirectoryComposioConnector, connections: DirectoryComposioConnection[] = []): string {
  ...
  const accountHint = firstActive?.wordId
    ? t(' If there are multiple accounts, prefer `{wordId}`.', { wordId: firstActive.wordId })
    : ''
  return `${t('Use the Composio CLI skill with the {name} connector ({slug}). Start by listing what it can do here, mention the current connection status, and suggest one safe command I can run now.', { name: connector.name, slug: connector.slug })}${accountHint}`
}
...
async function loadApps(): Promise<void> {
  ...
  try {
    ...
    appError.value = formatAppsLoadError(error)
  } finally {
    isLoadingApps.value = false
  }
}

function formatAppsLoadError(error: unknown): string {
  const message = error instanceof Error ? error.message : ''
  if (/cloudflare|403 forbidden|challenge-error-text|connectors\/directory/i.test(message)) {
    return t('The official app catalog is temporarily unavailable. Try again later.')
  }
  return message || t('Failed to load apps')
}

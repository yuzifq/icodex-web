...
type CurrentModelConfig = {
  ...
  speedMode: SpeedMode
  runtimeConfig: CodexRuntimeConfig | null
}

export type ModelReasoningEffortSettings = {
...
export async function resumeThread(threadId: string): Promise<ResumedThread> {
  ...
  const promise = (async () => {
    ...
    return {
      ...
      runtimeConfig: runtimeConfigFromThreadSettings(payload.approvalPolicy, payload.sandbox),
      messages,
      inProgress: readThreadInProgressFromResponse(payload),
      activeTurnId: readActiveTurnIdFromResponse(payload),
      hasMoreOlder: startTurnIndex > 0,
...
export async function getAvailableModelIds(options: { includeProviderModels?: boolean; requireProviderModels?: boolean; providerId?: string } = {}): Promise<string[]> {
  ...
  const payload = await callRpc<ModelListResponse>('model/list', { includeHidden: true })
  const ids: string[] = []
  for (const row of payload.data) {
    const candidate = row.id || row.model
    if (!candidate || HIDDEN_MODEL_IDS.has(candidate.trim().toLowerCase()) || ids.includes(candidate)) continue
    ids.push(candidate)
  }
  ...
  for (const candidate of providerModels.ids) {
    if (!HIDDEN_MODEL_IDS.has(candidate.trim().toLowerCase()) && !ids.includes(candidate)) ids.push(candidate)
  }
  return ids
}

export async function getModelReasoningEffortSettings(modelId: string): Promise<ModelReasoningEffortSettings> {
  const normalizedModelId = modelId.trim()
  if (!normalizedModelId) return { efforts: [], defaultEffort: '', supportsFast: false }
  const payload = await callRpc<ModelListResponse>('model/list', { includeHidden: true })
  const model = payload.data.find((candidate) => (candidate.id || candidate.model).trim() === normalizedModelId)
  if (!model) return { efforts: [], defaultEffort: '', supportsFast: false }
  const efforts = model.supportedReasoningEfforts
    .map((option) => normalizeReasoningEffort(option.reasoningEffort))
    .filter((effort, index, values): effort is ReasoningEffort => Boolean(effort) && values.indexOf(effort) === index)
  const speedMetadata = model as unknown as { additionalSpeedTiers?: unknown, serviceTiers?: unknown }
  const supportsFast =
    (Array.isArray(speedMetadata.additionalSpeedTiers) && speedMetadata.additionalSpeedTiers.includes('fast')) ||
    (Array.isArray(speedMetadata.serviceTiers) && speedMetadata.serviceTiers.some((tier) => typeof tier === 'object' && tier !== null && (tier as { id?: unknown }).id === 'priority'))
  return { efforts, defaultEffort: normalizeReasoningEffort(model.defaultReasoningEffort), supportsFast }
}
...
export async function getCurrentModelConfig(): Promise<CurrentModelConfig> {
  ...
  return {
    ...
    reasoningEffort,
    speedMode,
    runtimeConfig: runtimeConfigFromConfig(payload.config),
  }
}

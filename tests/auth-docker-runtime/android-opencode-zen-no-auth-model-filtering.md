...
3. Install the tarball into the Android proot through `/Users/tester/Git-projects/codex-web-local-android/andClaw-codex/ssh.sh`.
4. Remove `~/.codex/auth.json` and `~/.codex/webui-custom-providers.json` inside the Android proot.
5. Start `codexui --port 18935 --no-password --no-open --no-login` inside the Android proot.

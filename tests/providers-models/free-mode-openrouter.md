...
#### Feature
Select the OpenRouter provider in settings and enter your own OpenRouter API key. Default model is `openrouter/free` — OpenRouter's meta-model that auto-routes to the least-loaded free model, avoiding per-model rate limits. Model selector shows only free models when OpenRouter is active. Config is isolated from `~/.codex/config.toml` — state stored in `~/.codex/webui-custom-providers.json` and passed to app-server via `-c` CLI args.
...
4. Select **OpenRouter** as provider and enter your own OpenRouter API key.
5. Verify the provider turns on and model dropdown changes to `openrouter/free`.
...
- `POST /codex-api/free-mode/custom-key` — body `{ "key": "sk-or-v1-..." }` — sets a user-provided OpenRouter API key.
...
- Clearing the custom key disables OpenRouter until another key is provided.
...
- No shared or embedded API keys are included in the public repository.
- A user-provided API key can be set to use OpenRouter free-tier models when available.

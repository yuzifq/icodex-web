...
3. Keep a valid host auth file at `/Users/tester/.codex/auth.json`.
...
5. Copy `/Users/tester/.codex/auth.json` into the isolated `CODEX_HOME` while the Vite server is still running, reload the app, and confirm the new-chat composer switches from `big-pickle` to a Codex/GPT model.
